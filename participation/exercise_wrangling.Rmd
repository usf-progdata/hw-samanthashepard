...
4. Put `continent` first (in the data frame). Hint: use the `everything()` function.
...
#within each year, order by life expectancy
...
#across = across a bunch of different columns, do the same thing.
#".x" = represents all of the values for A1:O5.
```
...
gapminder %>% 
  ...
  summarize(mu = mean(lifeExp),
            sigma = sd(lifeExp))
#this gives you a mean and sd for each group for each of the years
```
...
gapminder %>% 
  ...
  ungroup()
#mean centering
#can get rid of the year trend and just compare countries
```
...
psych::bfi %>%
  ...
  ungroup()
#ungroup - lets you go back to working with the whole data set,rather than the group you were just working with
```
