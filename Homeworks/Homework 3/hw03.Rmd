...
green_data_missing_r_total <- green_data_missing_r %>% 
  rowwise() %>% 
  mutate(
          GreenRep = mean(green1, green2r & green3:green5, trim = 0, na.rm = TRUE),
          Compassion = mean(comp1:comp5 & comp6r:comp10r, trim = 0, na.rm = TRUE),
          IntCur = mean(intel1:intel6 & intel7r:intel10r, trim = 0, na.rm = TRUE),
          Open = mean(open1:open6 & open7r:open10r, trim = 0, na.rm = TRUE)) %>% 
